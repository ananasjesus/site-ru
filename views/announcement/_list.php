...
<div class="news-item">
    <div class="card" style="height: 26rem; width: 90%; border:2px solid #b4b2b3; border-radius: 8px; background-color: #5b92cb">
        <div class="card-header">
            <?= strlen($model->title) < 100 ? $model->title : mb_substr($model->title, 0, 100) . '...' ?>
        </div>
        <div class="card-body" style="margin: 1rem;">
            ...
            <p class="card-text"><?= strlen($model->content) < 300 ? $model->content : mb_substr($model->content, 0, 300) . '...'?></p>
